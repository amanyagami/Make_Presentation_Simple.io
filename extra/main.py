# main.py (updated with per-upload folders)
import os
import re
import json
import uuid
...
async def write_status(upload_id: str, data: dict):
    ...
    """
    per_upload_dir = UPLOAD_DIR / upload_id
    per_upload_dir.mkdir(parents=True, exist_ok=True)
    status_path = per_upload_dir / "status.json"
...
# ----------------------------
# Image extraction helpers (external extractor integration) - kept for compatibility
# ----------------------------
...
def collect_and_rename_extracted_images_from_paths(paths: List[Path]) -> List[Dict[str, Any]]:
    """
    Legacy helper - kept but no longer used (figures now live inside per-upload folder).
    """
    # This function is not called in the current flow.
    # It is retained unchanged for potential future external extractor use.
    return []
...
async def process_upload_background(upload_id: str, pdf_path: Path, basename: str, figures_index: List[Dict[str, Any]]):
    ...
    All artifacts are now stored inside uploads/<upload_id>/.
    """
    per_upload_dir = UPLOAD_DIR / upload_id
    per_upload_dir.mkdir(parents=True, exist_ok=True)

    # Keep references to intermediate files so cleanup can target them
    vlm_raw_out_path = per_upload_dir / "vlm.raw.txt"
    raw_out_path = per_upload_dir / "llm.raw.txt"
    text_path = ""
    ...
    try:
        ...
        # Persist VLM raw output for debugging (inside per-upload folder)
        try:
    ...
    "  - Preserve image references that point to /uploads/<upload_id>/* when present. If an image reference uses a placeholder like \"<Image 1>\" that should already be resolved by the VLM step; keep it or replace it with the web path /uploads/<upload_id>/<filename>.\n"
    ...
    + (raw_text_for_prompt or "")
        ...
        # Persist LLM raw output for debugging (inside per-upload folder)
        try:
        ...
        # --- 5) Normalize/resolve image_ref to concrete /uploads/<upload_id>/* paths ---
        ...
        ordered_web_paths = [f["path"] for f in figures_index]
        ...
        for s in data.get("slides", []):
            ...
            if isinstance(img, str):
                ...
                if img in id_to_path:
                    ...
                    continue
                # If it's a web path already, keep it
        ...
        # --- 6) Persist final parsed JSON ---
        parsed_out_path = per_upload_dir / "final.json"
        try:
        ...
        # --- 7) Copy viewer.html → index.html so /uploads/<upload_id>/ renders the finished deck ---
        viewer_src = APP_DIR / "viewer.html"
        index_dest = per_upload_dir / "index.html"
        if viewer_src.exists():
            try:
                shutil.copy(str(viewer_src), str(index_dest))
                logger.info("Copied viewer.html → %s (self-contained deck viewer ready)", index_dest)
            except Exception:
                logger.exception("Failed to copy viewer.html to upload folder")
        else:
            logger.warning("viewer.html not found in APP_DIR - per-upload index.html was not created")

        # --- 8) Cleanup intermediate previews and text artifacts (safe to delete now) ---
        try:
            # 8a) Remove previews directory for this upload_id
            ...
            # 8b) Remove extracted text file if extract_pdf_to_text returned a path
            if text_path:
                try:
                    tp = Path(text_path)
            ...
            # 8c) Remove intermediate raw VLM and LLM text outputs (but KEEP final.json + figures + index.html)
            if vlm_raw_out_path.exists():
                try:
        ...
        # Write final status (manifest and viewer are now available inside the upload folder)
        await write_status(upload_id, {
            "upload_id": upload_id,
            "state": "done",
            "step": "finished",
            ...
            "progress": 100,
            "manifest_url": f"/uploads/{upload_id}/final.json",
            "viewer_url": f"/uploads/{upload_id}/"
        })
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    Accept PDF uploads:
      - reject non-PDFs,
      - create per-upload folder uploads/<upload_id>/,
      - save file to uploads/<upload_id>/<original-filename>,
    ...
    save_name = file.filename  # original name (folder provides isolation)

    # Create per-upload folder
    per_upload_dir = UPLOAD_DIR / upload_id
    per_upload_dir.mkdir(parents=True, exist_ok=True)

    save_path = per_upload_dir / save_name
...
async def start_processing(upload_id: str, payload: Dict[str, Any]):
    ...
    All figures are now saved directly inside uploads/<upload_id>/figureN.png
    """
    try:
        per_upload_dir = UPLOAD_DIR / upload_id
        if not per_upload_dir.exists():
        ...
        save_path = per_upload_dir / pdf_filename
        ...
        # Figures are stored directly inside the per-upload folder (no global figures/ dir)
...
async def status(upload_id: str):
    """
    Return the status JSON for an upload.
    """
    per_upload_dir = UPLOAD_DIR / upload_id
    status_path = per_upload_dir / "status.json"
...
async def list_upload_artifacts(upload_id: str):
    ...
    All files (final.json, figures, index.html, etc.) are now inside the per-upload folder.
    """
    per_upload_dir = UPLOAD_DIR / upload_id
    files = [p.name for p in per_upload_dir.glob("*")] if per_upload_dir.exists() else []
    previews_subdir = PREVIEWS_DIR / upload_id
    previews = [p.name for p in previews_subdir.glob("*")] if previews_subdir.exists() else []
    return JSONResponse(status_code=200, content={
        "files": files,
        "previews": previews,
        "figures": [f for f in files if f.startswith("figure")]
    })