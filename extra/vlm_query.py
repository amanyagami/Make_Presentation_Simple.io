...
def generate_multimodal_response(
    ...
    user_instruction = (
        " how many slides are required to explain the full story of the text ? "+ " text =  " + " :".join(raw_text) + " : \n"
        "understand  the text fully and create 2 kinds of slides. type1: without image, type2: with images . both should have content. if image slide it must explain the image, its impact\n"
        "slides must take all the images and explain the image showing impact/ explanation.\n\n"
        " the flow should be able to explain the full story "
        "Each slide's content should be concise; explain impact in 3-4 bullet points inside the slide object's steps as separate step entries.\n"
        "the slides should end with a thank you  and start with a title only slide \n" 
        "output in below slide structure\n"
        ...
        "When you reference an image in a slide object, set \"image_ref\" to the placeholder string (e.g. \"<Image 1>\").\n"
        "Do not include any text outside the single JSON object/array. The JSON must validate."
    )
